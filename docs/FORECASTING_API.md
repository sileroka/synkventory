...
### GET `/api/v1/forecasting/reorder-suggestions`
...
curl -H "X-Tenant-Slug: demo" \
  "http://localhost:8000/api/v1/forecasting/reorder-suggestions?lead_time_days=7"
```

### POST `/api/v1/forecasting/items/{item_id}/moving-average`
...
curl -X POST -H "X-Tenant-Slug: demo" \
  "http://localhost:8000/api/v1/forecasting/items/ITEM_UUID/moving-average?window_size=7&periods=14"
```

### POST `/api/v1/forecasting/items/{item_id}/exp-smoothing`
...
```bash
curl -X POST -H "X-Tenant-Slug: demo" \
  "http://localhost:8000/api/v1/forecasting/items/ITEM_UUID/exp-smoothing?window_size=7&periods=14&alpha=0.3"
```
