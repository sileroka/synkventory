...
def serialize_bom_entry(entry: BillOfMaterialModel) -> dict:
    ...
    if component:
        component_data = {
            ...
            "image_url": (
                storage_service.get_signed_url(component.image_key)
                if component.image_key
                else None
            ),
        }
...
def serialize_where_used_entry(entry: BillOfMaterialModel) -> dict:
    ...
    if parent:
        parent_data = {
            ...
            "image_url": (
                storage_service.get_signed_url(parent.image_key)
                if parent.image_key
                else None
            ),
        }
...
def add_bom_component(
    ...
    try:
        ...
        bom_entries = bom_service.get_bom_components(db, item_id)
        created_entry = next(
            (e for e in bom_entries if e.id == bom_entry.id), bom_entry
        )
...
@router.get(
    "/items/{item_id}/where-used", response_model=DataResponse[list[WhereUsedEntry]]
)
def get_where_used(
    request: Request,
    item_id: UUID,
...
@router.get(
    "/items/{item_id}/bom/availability", response_model=DataResponse[BOMAvailability]
)
...
@router.post(
    "/items/{item_id}/bom/unbuild", response_model=DataResponse[BOMUnbuildResult]
)
